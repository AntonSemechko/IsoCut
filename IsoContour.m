...
%   - F     : N-by-1 array containing values of a scalar field sampled at
%             the  mesh vertices. 
...
%             flag=1 if v1 and v2 are on a boundary edge and is zero 
%             otherwise. Note, closed surfaces do not have boundaries and
%             thus flag=0 for all k and m. Linear interpolation convention 
%             used here is x(t) = (1-t)*x1 + t*x2, where x1 and x2
%             correspond to v1 and v2, resp.
%   - H     : handles of the plotted level sets.
%
...
F = F(:);
if ~isnumeric(F) || length(F)~=size(X,1) %|| ~all(isfinite(F))
...
t_tol = 1e-12;
for i = 1:n
    chk = chk | abs(F-iv(i))<f_tol;
end

chk_edges = false;
if any(chk) % any vertices coincident with the level set(s)?
    Eo = sort(edges(TR),2);
    [Fe1,Fe2] = deal(F(Eo(:,1)),F(Eo(:,2)));
    id_eq = abs(Fe1-Fe2)<=f_tol; % edges whose verices have equal field values
    if any(id_eq)
        chk_edges = true;
        Eo_eq = Eo(id_eq,:);
...
for i = 1:n % loop through level sets
    ...
    if ~any(f_id) && ~chk_edges
        fprintf(2,'Level set %10e does not exist\n',iv(i))
        continue
    end
    e_id = e_id(f_id,:);    
    ...
    id_slf = abs(dFe)<=f_tol;
    if any(id_slf(:)) % check if there are edges coincident with the level set
        t(id_slf) = 0;
    end
    ...
    id_12 = e_id(:,1) & e_id(:,2) & sqrt(sum((P1-P2).^2,2))>1E-12; % E1--E2
    id_13 = e_id(:,1) & e_id(:,3) & sqrt(sum((P1-P3).^2,2))>1E-12; % E1--E3
    id_23 = e_id(:,2) & e_id(:,3) & sqrt(sum((P2-P3).^2,2))>1E-12; % E2--E3
    ...
    if chk_edges
        ...
        e_id = abs(Fe1-iv(i))<=f_tol & id_eq;
        chk_match = any(e_id);
        if chk_match
            
            % Revome duplicate edges
            [t1,t2] = deal(VTi(1:3:end,3),VTi(2:3:end,3));
            idx = (t1<=t_tol | t1>=(1-t_tol)) & (t2<=t_tol | t2>=(1-t_tol));
            if any(idx) 
    ...
    if ~isempty(FB) && nargout>2
        ...
        if chk_match
            ...
            idx = (idx(:,1) & VTi(:,3)<=t_tol) | (idx(:,2) & VTi(:,3)>=(1-t_tol));
            bv_id = bv_id | idx;
        end
